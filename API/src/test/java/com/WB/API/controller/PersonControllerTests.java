...
//	}
	...
	@Test
	@DisplayName("Requête API pour charger une personne à partir d'un ID inexistant")
	void testGetPersonById_ReturnEmptyPerson() throws Exception {
		// Arrange
		when(personService.getPersonById(99)).thenThrow(new RessourceNotFoundException("Person not found"));

		// Act + Assert
		mockMvc.perform(get("/persons/id/99")).andExpect(status().isNotFound());
	}
